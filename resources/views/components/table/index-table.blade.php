<div class="mb-10">
    <h1 class="mb-4 text-xl font-medium">{{ __('Index table') }}</h1>
    <div class="bg-gray-200/60 p-4 lg:p-10">
        <p class="font-semibold text-sm mb-3">{{ __('Default table') }}</p>
        <x-dashui-index-table :headings="[
            ...
            <x-dashui-table-row :id="1020">
                ...
                <x-dashui-table-cell>{{ __('Jul 20 at 4:34pm') }}</x-dashui-table-cell>
                <x-dashui-table-cell>{{ __('Jaydon Stanton') }}</x-dashui-table-cell>
                <x-dashui-table-cell class="text-right">$969.44</x-dashui-table-cell>
                <x-dashui-table-cell><x-dashui-badge progress="complete">{{ __('Paid') }}</x-dashui-badge></x-dashui-table-cell>
                <x-dashui-table-cell><x-dashui-badge progress="incomplete">{{ __('Unfulfilled') }}</x-dashui-badge></x-dashui-table-cell>
                <x-dashui-table-cell class="text-center"><x-dashui-button as="link" href="#0" variant="plain">{{ __('Edit') }}</x-dashui-button></x-dashui-table-cell>
            </x-dashui-table-row>

            <x-dashui-table-row :id="1019">
                <x-dashui-table-cell class="font-bold">#1019</x-dashui-table-cell>
                <x-dashui-table-cell>{{ __('Jul 20 at 4:44pm') }}</x-dashui-table-cell>
                <x-dashui-table-cell>{{ __('Ruben Westerfelt') }}</x-dashui-table-cell>
                <x-dashui-table-cell class="text-right">$701.19</x-dashui-table-cell>
                <x-dashui-table-cell><x-dashui-badge progress="partiallyComplete">{{ __('Partially paid') }}</x-dashui-badge></x-dashui-table-cell>
                <x-dashui-table-cell><x-dashui-badge progress="incomplete">{{ __('Unfulfilled') }}</x-dashui-badge></x-dashui-table-cell>
                <x-dashui-table-cell class="text-center"><x-dashui-button as="link" href="#0" variant="plain">{{ __('Edit') }}</x-dashui-button></x-dashui-table-cell>
            </x-dashui-table-row>
            <x-dashui-table-row :id="1018">
                <x-dashui-table-cell class="font-bold">#1018</x-dashui-table-cell>
                <x-dashui-table-cell>{{ __('Jul 20 at 4:54pm') }}</x-dashui-table-cell>
                <x-dashui-table-cell>{{ __('Leo Carder') }}</x-dashui-table-cell>
        ...
        <p class="font-semibold text-sm mb-3 mt-5">{{ __('Table with row navigation url') }}</p>
        <x-dashui-index-table :headings="[
            ['title' => 'Order'],
            ['title' => 'Date',],
            ['title' => 'Customer'],
        ...
        <p class="font-semibold text-sm mb-3 mt-5">{{ __('Table with dropdown actions') }}</p>
        <x-dashui-index-table :headings="[
            ['title' => 'Order'],
            ['title' => 'Date',],
            ['title' => 'Customer'],
            ...
            <x-dashui-table-row :id="1020">
                ...
                <x-dashui-table-cell class="text-right">
                    <x-dashui-button type="button" variant="subtle" aria-controls="popover-table-1">{{ __('Options') }} <x-gmdi-expand-more class="w-4 h-4"/></x-dashui-button>
        ...
        <p class="font-semibold text-sm mb-3 mt-5">{{ __('Table with pagination') }}</p>
        <x-dashui-index-table :headings="[
            ['title' => 'Order'],
            ['title' => 'Date',],
            ['title' => 'Customer'],
        ...
        <p class="font-semibold text-sm mb-3 mt-5">{{ __('Table with filter') }}</p>
        <x-dashui-index-table :headings="[
            ['title' => 'Order'],
            ['title' => 'Date',],
            ['title' => 'Customer'],
            ...
            <x-slot:filter>
                <div class="flex flex-wrap items-center gap-x-2">
                    <x-dashui-button as="link" href="#0" variant="subtle" class="bg-neutral-200 hover:bg-neutral-200">{{ __('All') }}</x-dashui-button>
                    <x-dashui-button type="button" variant="subtle" aria-controls="popover-filter-1">{{ __('Payment') }} <x-gmdi-expand-more class="w-4 h-4"/></x-dashui-button>
                    <x-dashui-button type="button" variant="subtle" aria-controls="popover-filter-2">{{ __('Status') }} <x-gmdi-expand-more class="w-4 h-4"/></x-dashui-button>
                ...
                <div class="flex items-center gap-x-1">
                    ...
                    <x-dashui-popover id="popover-sort" role="dialog">
                        <span class="text-xs">{{ __('Sort by') }}</span>
